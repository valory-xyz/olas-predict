...
- [x] **Production-tree audit sweep.** Starting point was 30 unique high/critical advisories (`6 critical + 83 high` raw findings, mostly via `next@13.5.6` and the `wagmi → @walletconnect/*` chain). Cleared by bumping `next` `13.5.6` → `14.2.35` (clears CVE-2025-29927 critical authorization-bypass + 5 high advisories around SSRF, middleware authorization, cache poisoning, and Server Components DoS), bumping direct `lodash` `4.17.21` → `4.18.1`, and adding Yarn `resolutions` for 13 transitive packages (`@coinbase/wallet-sdk`, `base-x`, `braces`, `cross-spawn`, `defu`, `elliptic`, `h3`, `lodash`, `node-forge`, `picomatch`, `secp256k1`, `sha.js`, `socket.io-parser`, `undici`). After the sweep: 0 critical, 2 high — both are RSC DoS advisories in `next` that require Next 15+ to clear. Allowlisted in [`.supply-chain/audit-allowlist.json`](./.supply-chain/audit-allowlist.json) with the rationale that this app uses Pages Router only (no `app/` directory, no `"use server"` / `"use client"` directives, no `middleware.ts`) so the vulnerable code paths are unreachable. Review date `2026-07-28`.
- [x] **Next.js 13 → 14 migration.** Bumped to 14.2.35. Required adding `staticPageGenerationTimeout: 300` to [`next.config.js`](./next.config.js) — Next 14 collects per-page module data via worker subprocesses with a default 60s timeout, and on Windows the worker startup overhead plus the wagmi/walletconnect import tree pushed `_app` past that limit. CI on Linux is much faster but the higher timeout costs nothing.
- [ ] **Next.js 14 → 15 migration** (separate PR, not supply-chain scope). Clears the 2 currently-allowlisted high advisories (1112653, 1116376). Both are RSC DoS paths and unreachable in this Pages-Router app, which is why it's safe to allowlist for the migration window. Next 15 defaults to React 19 (still supports React 18 as a peer); a fuller compat review of the AntD / styled-components / wagmi / recharts stack against React 19 should accompany the bump.

...
All GitHub Actions used in [.github/workflows/main.yml](./.github/workflows/main.yml) are SHA-pinned: `actions/checkout` and `actions/setup-node` on every Node-using job, plus `actions/checkout` on the gitleaks `scan` job. The gitleaks binary itself is downloaded by URL with `curl -fsSL` and verified against a known SHA-256 (`sha256sum -c`) before install, so a compromised release on the upstream `zricethezav/gitleaks` repo would not silently land in CI. Currently pinned to **gitleaks v8.30.1**; the SHA-256 was sourced from the `gitleaks_8.30.1_checksums.txt` file published alongside the release tarball. Gitleaks does not ship a PGP signature on its checksums file, so HTTPS-served `checksums.txt` plus the pinned hash in the workflow is the realistic trust chain. Audit the pins periodically — at minimum once per major release of each action — until a bot with `pinDigests: true` is in place.

Every job in [.github/workflows/main.yml](./.github/workflows/main.yml) declares a job-level `permissions:` block: `contents: read` for the four jobs that touch repo content (`build`, `audit`, `lockfile-lint`, `scan`, `install-hooks`) and `permissions: {}` for the `all-checks-passed` aggregator (which only reads `needs.*.result`). The default `GITHUB_TOKEN` has broad read+write across the repo's API surface; the explicit per-job declarations narrow each job's token to the minimum it needs, so a hostile transitive dep or a compromised action cannot write to the repo, comment on PRs, or read secrets it has no reason to touch.

### 5. Audit in CI

The `audit` job in [.github/workflows/main.yml](./.github/workflows/main.yml) runs `yarn audit:prod` on every PR. That script ([`scripts/audit.mjs`](./scripts/audit.mjs)) wraps `yarn audit --groups dependencies --json`, applies the high/critical gate explicitly (Yarn 1.x's `--level` only filters the *printed* output — see the "Yarn 1.x audit quirk" note below), and consults [`.supply-chain/audit-allowlist.json`](./.supply-chain/audit-allowlist.json). The allowlist file uses the fleet-standard schema: a top-level `entries` array of objects with `id` (numeric npm advisory ID — required for matching), `package`, `severity`, `reason`, `added` and `review` dates, plus an optional human-readable `ghsa`. The file also carries a `_fields` block that self-documents the schema so the next contributor adding a suppression has the spec in front of them. An unallowlisted high/critical advisory against a production dependency blocks merge — the PR author must either (a) bump the dep, (b) add an exact-pinned Yarn `resolutions` entry per [§1](#1-exact-version-pinning-in-packagejson) with the advisory ID in the PR description, or (c) add an entry to `entries[]` with a stated reason and a review date. Allowlist entries past their `review` date generate a `::warning::` in CI output (surfaced in the Actions UI) but do not fail the job — the warning is how the team is kept honest about re-evaluating suppressions. Allowlist entries that no longer match a current advisory also surface a `::warning::` so cleared entries get pruned. The wrapper script fails loudly if it encounters the legacy `allowlist` key (the schema migrated to `entries` in May 2026) so a half-applied schema change can't silently no-op every suppression.
...
The `install-hooks` job in [.github/workflows/main.yml](./.github/workflows/main.yml) runs [`scripts/audit-install-hooks.mjs`](./scripts/audit-install-hooks.mjs) on every PR. The script walks `node_modules`, enumerates every package that declares a non-trivial `preinstall` / `install` / `postinstall` script, and diffs the list against [`.supply-chain/install-hooks.allowlist`](./.supply-chain/install-hooks.allowlist). Three failure modes block merge:

1. **New unexpected hook** — a transitive dep added a script not in the allowlist. This is the precise shape of a supply-chain compromise (benign prebuilt-binary download flipping to `curl ... | sh` is the canonical case).
2. **Missing hook** — a previously-allowlisted package no longer ships an install script. Usually indicates drift after a dep removal; refresh the allowlist.
3. **Changed hook** — a package still in the tree but its script command mutated. Inspect carefully, same threat model as the first case.
...
[`.yarnrc`](./.yarnrc) sets `--install.ignore-scripts true` at the workspace level, so local `yarn install` runs do not execute postinstall scripts by default. This is install-time defense against the threat-model scenario in [§ Threat model #3](#threat-model) — a freshly-published malicious version of a transitive dep can run code on a contributor's machine the moment they `yarn install`.

The flip side: first-time installs of native modules (`keccak`, `secp256k1`, `bufferutil`, `sharp`) will fail to build their native binaries because the postinstall did not run. To opt back in narrowly, on demand:

```
yarn install --ignore-scripts=false
```

Use this only after reading the package's install script in `node_modules/<pkg>/package.json` and confirming it does what you expect. The install-hook diff gate ([§6a](#6a-install-hook-diff-gate)) is the durable enforcement layer — repo-wide install scripts are the day-one shield, but the CI gate is what catches a *new* postinstall arriving in a future dep bump.
...
- [x] SHA-pin the `gitleaks` job in [.github/workflows/main.yml](./.github/workflows/main.yml). Bumped `actions/checkout@v3` → `@v4.3.1` (SHA-pinned), removed the unused `actions/setup-go@v3` step (the job runs a precompiled gitleaks binary, never invokes `go`), switched the binary download from `wget` to `curl -fsSL`, and added `sha256sum -c` verification against a known checksum before install. Currently pinned to gitleaks v8.30.1 (May 2026 bump from v8.10.1; SHA-256 sourced from the upstream release's `checksums.txt`).
- [x] **Install-hook diff gate.** [`scripts/audit-install-hooks.mjs`](./scripts/audit-install-hooks.mjs) + [`.supply-chain/install-hooks.allowlist`](./.supply-chain/install-hooks.allowlist) + the `install-hooks` CI job. Catches new transitive postinstall scripts that `yarn audit` does not see. See [§6a](#6a-install-hook-diff-gate).
- [x] **Local install scripts opt-out by default.** [`.yarnrc`](./.yarnrc) sets `--install.ignore-scripts true`. See [§6b](#6b-local-install-scripts-policy).
- [x] **Allowlist schema migration.** `.supply-chain/audit-allowlist.json` migrated from the local `allowlist` key to the fleet-standard `entries` key with the `_fields` self-documentation block. `scripts/audit.mjs` reads `entries` and rejects the legacy `allowlist` key with a clear migration error. See [§5](#5-audit-in-ci).
- [x] **Job-level CI permissions.** Every job in [.github/workflows/main.yml](./.github/workflows/main.yml) declares `permissions: contents: read` (or `permissions: {}` for the aggregator), narrowing the GITHUB_TOKEN to the minimum each job needs. See the SHA-pin paragraph in [§4](#4-cooldown-window-on-updates).
